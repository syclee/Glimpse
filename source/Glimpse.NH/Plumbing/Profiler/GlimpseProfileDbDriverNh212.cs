...
{
    public class GlimpseProfileDbDriverNh212<TInnerDriver> : IDriver, IEmbeddedBatcherFactoryProvider, ISqlParameterFormatter
        where TInnerDriver : class, IDriver, new()
    {
        private readonly TInnerDriver _innerDriver;

        public GlimpseProfileDbDriverNh212()
            : this(new TInnerDriver())
        {
        }

        public GlimpseProfileDbDriverNh212(TInnerDriver innerDriver)
        {
            if (innerDriver == null)
