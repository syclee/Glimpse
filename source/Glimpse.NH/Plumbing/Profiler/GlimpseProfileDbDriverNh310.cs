...
{
    public class GlimpseProfileDbDriverNh310<TInnerDriver> : IDriver, IEmbeddedBatcherFactoryProvider, ISqlParameterFormatter
        where TInnerDriver : class, IDriver, new()
    {
        private readonly TInnerDriver _innerDriver;

        public GlimpseProfileDbDriverNh310()
            : this(new TInnerDriver())
        {
        }

        public GlimpseProfileDbDriverNh310(TInnerDriver innerDriver)
        {
            if (innerDriver == null)
